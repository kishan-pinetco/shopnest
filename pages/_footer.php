<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <!-- Tailwind Script  -->
    <script src="https://cdn.tailwindcss.com?plugins=forms,typography,aspect-ratio,line-clamp"></script>

    <!-- google fonts -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@100..900&display=swap" rel="stylesheet">

    <!-- favicon -->
    <link rel="shortcut icon" href="../src/logo/favicon.png">

    <style>
        .outfit {
            font-family: "Outfit", sans-serif;
            font-optical-sizing: auto;
            font-weight: 500;
            font-style: normal;
        }
    </style>
</head>

<body>
    <footer class="outfit space-y-10 py-5 px-10">
        <div class="grid grid-cols-2 gap-y-5 gap-x-10 sm:flex sm:justify-between bg-purple-500">
            <div class=" space-y-4">
                <h1 class="text-xl text-indigo-500">Get to know us</h1>
                <div class="flex flex-col gap-y-2 text-sm translate-x-1">
                    <a href="">About Us</a>
                    <a href="">Contacs Us</a>
                    <a href="">Investors</a>
                </div>
            </div>
            <div class="space-y-4">
                <h1 class="text-xl text-indigo-500">Customer service</h1>
                <div class="flex flex-col gap-y-2 text-sm translate-x-1">
                    <a href="">Help Center</a>
                    <a href="">faq</a>
                    <a href="">Payment Method</a>
                </div>
            </div>
            <div class="space-y-4">
                <h1 class="text-xl text-indigo-500">Orders & Returns</h1>
                <div class="flex flex-col gap-y-2 text-sm translate-x-1">
                    <a href="">Track</a>
                    <a href="">Shipping & Delivery</a>
                    <a href="">Return & Exchange</a>
                </div>
            </div>
        </div>
        <div class="flex justify-center bg-blue-400">
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-12">
                <div class="flex items-center gap-3">
                    <span><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
                            <path d="M16 1c-8.28 0-15 6.72-15 15s6.72 15 15 15 15-6.72 15-15c0-8.28-6.72-15-15-15zM16 29c-7.17 0-13-5.83-13-13s5.83-13 13-13 13 5.83 13 13-5.83 13-13 13z"></path>
                            <path d="M16 8c-4.41 0-8 3.59-8 8s3.59 8 8 8h1v-2h-1c-3.31 0-6-2.69-6-6s2.69-6 6-6 6 2.69 6 6v1.5c0 0.55-0.45 1-1 1s-1-0.45-1-1v-4.75h-1.69c-0.65-0.47-1.45-0.75-2.31-0.75-2.21 0-4 1.79-4 4s1.79 4 4 4c0.94 0 1.8-0.34 2.48-0.89 0.53 0.83 1.46 1.39 2.52 1.39 1.65 0 3-1.35 3-3v-1.5c0-4.41-3.59-8-8-8zM16 18c-1.1 0-2-0.9-2-2s0.9-2 2-2c1.1 0 2 0.9 2 2s-0.9 2-2 2z"></path>
                        </svg></span>
                    <div class="text-sm">
                        <p>Didn't find what you were looking for?</p>
                        <a href="" class="underline">Contact Us</a>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <span><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
                            <path d="M16 1c-8.28 0-15 6.72-15 15s6.72 15 15 15c8.28 0 15-6.72 15-15s-6.72-15-15-15zM19.45 20.9c-0.27 0.19-0.55 0.35-0.84 0.49-0.79 0.39-1.67 0.61-2.61 0.61s-1.82-0.22-2.61-0.61c-0.29-0.14-0.57-0.31-0.84-0.49-0.56-0.4-1.050-0.89-1.45-1.45-0.19-0.26-0.35-0.55-0.49-0.84-0.39-0.79-0.61-1.67-0.61-2.61s0.22-1.82 0.61-2.61c0.14-0.29 0.31-0.57 0.49-0.84 0.4-0.56 0.89-1.050 1.45-1.45 0.26-0.19 0.55-0.35 0.84-0.49 0.79-0.39 1.67-0.61 2.61-0.61s1.82 0.22 2.61 0.61c0.29 0.14 0.58 0.31 0.84 0.49 0.56 0.4 1.050 0.89 1.45 1.45 0.19 0.27 0.35 0.55 0.49 0.84 0.39 0.79 0.61 1.67 0.61 2.61s-0.22 1.82-0.61 2.61c-0.14 0.29-0.31 0.58-0.49 0.84-0.4 0.56-0.89 1.050-1.45 1.45zM23.44 18.91c0.36-0.9 0.56-1.88 0.56-2.91s-0.2-2.010-0.56-2.91l4.030-3.18c0.98 1.81 1.53 3.89 1.53 6.090s-0.55 4.28-1.52 6.1l-4.040-3.19zM26.39 8.22l-3.92 3.090c-0.5-0.68-1.090-1.28-1.78-1.78l3.090-3.92c0.99 0.74 1.87 1.62 2.61 2.61zM16 3c2.2 0 4.28 0.55 6.090 1.52l-3.18 4.030c-0.9-0.35-1.88-0.55-2.91-0.55s-2.010 0.2-2.91 0.56l-3.18-4.040c1.81-0.97 3.89-1.52 6.090-1.52zM8.22 5.61l3.090 3.92c-0.68 0.5-1.28 1.090-1.78 1.78l-3.92-3.090c0.74-0.99 1.62-1.87 2.61-2.61zM4.52 9.9l4.030 3.18c-0.35 0.91-0.55 1.89-0.55 2.92s0.2 2.010 0.56 2.91l-4.040 3.19c-0.97-1.82-1.52-3.9-1.52-6.1s0.55-4.28 1.52-6.1zM5.61 23.78l3.92-3.090c0.5 0.68 1.090 1.28 1.78 1.78l-3.090 3.92c-0.99-0.74-1.87-1.62-2.61-2.61zM16 29c-2.2 0-4.28-0.55-6.090-1.52l3.18-4.030c0.9 0.35 1.88 0.55 2.91 0.55s2.010-0.2 2.91-0.56l3.18 4.030c-1.81 0.98-3.89 1.53-6.090 1.53zM23.78 26.39l-3.090-3.92c0.68-0.5 1.28-1.090 1.78-1.78l3.92 3.090c-0.74 0.99-1.62 1.87-2.61 2.61z"></path>
                        </svg></span>
                    <div class="text-sm">
                        <p>How can we help you today?</p>
                        <a href="" class="underline">Help Center</a>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <span><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
                            <path d="M16 3c7.17 0 13 5.83 13 13s-5.83 13-13 13-13-5.83-13-13 5.83-13 13-13zM16 1c-8.28 0-15 6.72-15 15s6.72 15 15 15 15-6.72 15-15-6.72-15-15-15v0z"></path>
                            <path d="M11 15c-1.65 0-3-1.35-3-3s1.35-3 3-3 3 1.35 3 3-1.35 3-3 3zM11 11c-0.55 0-1 0.45-1 1s0.45 1 1 1 1-0.45 1-1-0.45-1-1-1z"></path>
                            <path d="M18.5 12h5v2h-5v-2z"></path>
                            <path d="M16 25c-1.17 0-2.33-0.23-3.44-0.69l0.77-1.85c1.73 0.71 3.63 0.71 5.36 0 1.73-0.72 3.070-2.060 3.79-3.79l1.85 0.77c-0.92 2.22-2.65 3.95-4.87 4.87-1.13 0.46-2.29 0.69-3.46 0.69z"></path>
                        </svg></span>
                    <div class="text-sm">
                        <p>We'd love to hear what you think!</p>
                        <a href="" class="underline">Give Feedback</a>
                    </div>
                </div>
            </div>
        </div>
        <div class="lg:flex justify-between items-center bg-gray-500">
            <div class="flex items-center gap-5">
                <div class="flex items-center justify-start">
                    <img class="translate-x-3 translate-y-2 w-24" src="../src/logo/logo3.svg" alt="logo">
                    <svg width="125" height="60" viewBox="0 0 355 101.76409801109908" class="looka-1j8o68f">
                        <defs id="SvgjsDefs2249"></defs>
                        <g id="SvgjsG2250" featurekey="k6v3K5-0" transform="matrix(3.263098946758052,0,0,3.263098946758052,1.0842826642604835,-11.576542400576212)" fill="#000000">
                            <path d="M10.34 10.6 l-1.38 1.6 q-1.1 -0.9 -3.08 -0.9 q-0.78 0 -1.44 0.34 t-0.66 0.78 q0 0.36 0.14 0.54 q0.18 0.26 0.64 0.36 q0.52 0.14 1.48 0.14 q2.08 0 3.28 0.46 q1.06 0.38 1.48 1.14 q0.34 0.62 0.34 1.58 q0 1.8 -1.5 2.76 q-1.42 0.92 -3.74 0.92 q-1.34 0 -2.59 -0.59 t-2.11 -1.61 l1.48 -1.5 l0.18 0.2 q0.42 0.44 0.68 0.68 q0.46 0.38 0.96 0.58 q0.62 0.24 1.4 0.24 q1.44 0 2.32 -0.44 t0.88 -1.24 q0 -0.68 -0.78 -0.92 q-0.7 -0.22 -2.42 -0.22 q-2 0 -3.06 -0.7 q-1.22 -0.76 -1.22 -2.38 q0 -0.92 0.6 -1.64 q0.56 -0.7 1.54 -1.09 t2.14 -0.39 q1.24 0 2.22 0.26 q1.16 0.3 2.22 1.04 z M16.78 15.879999999999999 l0 4.12 l-2.04 0 l0 -10.36 l2.04 0 l0 4.14 q0.54 -0.26 1.22 -0.26 q0.4 0 1.42 0.14 q1.16 0.16 1.7 0.16 q0.76 0 1.3 -0.26 l0 -3.92 l2.04 0 l0 10.36 l-2.04 0 l0 -4.34 q-0.52 0.26 -1.22 0.26 q-0.54 0 -1.6 -0.14 q-1.02 -0.14 -1.52 -0.14 q-0.74 0 -1.3 0.24 z M38.34 14.74 q0 -1.1 -0.6 -1.9 q-0.52 -0.7 -1.46 -1.08 q-0.86 -0.34 -1.9 -0.34 t-1.92 0.36 q-0.94 0.38 -1.46 1.12 q-0.58 0.8 -0.58 1.92 t0.6 1.92 q0.52 0.72 1.46 1.12 q0.88 0.36 1.9 0.36 t1.92 -0.4 q0.92 -0.42 1.46 -1.18 q0.58 -0.82 0.58 -1.9 z M40.5 14.719999999999999 q0 1.7 -0.9 3.02 q-0.82 1.22 -2.26 1.92 q-1.38 0.66 -2.98 0.66 t-2.98 -0.66 q-1.42 -0.68 -2.24 -1.88 q-0.88 -1.32 -0.88 -3 t0.9 -2.98 q0.82 -1.2 2.26 -1.86 q1.38 -0.64 2.98 -0.64 t2.96 0.6 q1.42 0.64 2.24 1.82 q0.9 1.28 0.9 3 z M46.339999999999996 11.7 l0 3.1 q0.62 -0.36 1.52 -0.5 q0.68 -0.1 1.74 -0.1 l0.92 0 q0.84 0 1.18 -0.24 q0.46 -0.28 0.46 -1.06 q0 -0.7 -0.5 -0.98 q-0.38 -0.22 -1.14 -0.22 l-4.18 0 z M46.339999999999996 20 l-2.04 0 l0 -10.36 l6.58 0 q1 0 1.76 0.4 t1.19 1.15 t0.43 1.73 q0 1.54 -1 2.44 t-2.74 0.9 l-0.92 0 q-1.38 0 -2 0.12 q-0.56 0.1 -1.26 0.48 l0 3.14 z M58.1 7.18 l-1.74 -2.1 l2.82 0 l9.58 11.4 l0 -11.4 l2.2 0 l0 14.92 l-2.16 0 l-8.56 -10.26 l0 10.26 l-2.14 0 l0 -12.82 z M77.6 16.28 l0 1.64 l6.5 0 l0 2.08 l-8.54 0 l0 -10.36 l8.16 0 l0 2.06 l-6.12 0 l0 2.52 q0.62 -0.36 1.5 -0.5 q0.68 -0.1 1.74 -0.1 l1.52 0 l0 2.06 l-1.52 0 q-1.38 0 -2 0.12 q-0.54 0.1 -1.24 0.48 z M95.94 10.6 l-1.38 1.6 q-1.1 -0.9 -3.08 -0.9 q-0.78 0 -1.44 0.34 t-0.66 0.78 q0 0.36 0.14 0.54 q0.18 0.26 0.64 0.36 q0.52 0.14 1.48 0.14 q2.08 0 3.28 0.46 q1.06 0.38 1.48 1.14 q0.34 0.62 0.34 1.58 q0 1.8 -1.5 2.76 q-1.42 0.92 -3.74 0.92 q-1.34 0 -2.59 -0.59 t-2.11 -1.61 l1.48 -1.5 l0.18 0.2 q0.42 0.44 0.68 0.68 q0.46 0.38 0.96 0.58 q0.62 0.24 1.4 0.24 q1.44 0 2.32 -0.44 t0.88 -1.24 q0 -0.68 -0.78 -0.92 q-0.7 -0.22 -2.42 -0.22 q-2 0 -3.06 -0.7 q-1.22 -0.76 -1.22 -2.38 q0 -0.92 0.6 -1.64 q0.56 -0.7 1.54 -1.09 t2.14 -0.39 q1.24 0 2.22 0.26 q1.16 0.3 2.22 1.04 z M108.46 11.7 l-3.9 0 l0 8.3 l-2.06 0 l0 -8.3 l-3.76 0 l0 -2.06 l9.72 0 l0 2.06 z"></path>
                        </g>
                        <g id="SvgjsG2251" featurekey="SbIu4p-0" transform="matrix(1.664253779132007,0,0,1.664253779132007,19.070915348544332,67.87988925861347)" fill="#00ff00">
                            <path d="M6.08 5.84 q2.24 0 3.46 0.91 t1.24 2.57 q0 1.22 -0.67 2.04 t-1.81 1.16 l0 0.04 q1.38 0.12 2.29 1.11 t0.91 2.37 q0 1.88 -1.42 2.91 t-4.04 1.05 l-4.28 0 l0 -14.16 l4.32 0 z M5.84 11.84 q1.54 0 2.34 -0.55 t0.8 -1.75 q0 -2.1 -2.96 -2.14 l-2.58 0 l0 4.44 l2.4 0 z M6.36 18.44 q1.52 0 2.43 -0.67 t0.91 -1.79 q0 -1.28 -0.9 -1.93 t-2.46 -0.65 l-2.9 0 l0 5.04 l2.92 0 z M20.0615 5.84 l0 8.52 q0 2.16 0.92 3.28 t2.74 1.16 q1.82 -0.02 2.73 -1.15 t0.93 -3.21 l0 -8.6 l1.68 0 l0 8.82 q0 2.7 -1.4 4.19 t-3.94 1.51 q-2.5 0 -3.91 -1.51 t-1.43 -4.21 l0 -8.8 l1.68 0 z M36.803000000000004 5.84 l3.98 6.4 l4.1 -6.4 l2.02 0 l-5.28 8.06 l0 6.1 l-1.68 0 l0 -6.1 l-5.28 -8.06 l2.14 0 z M66.52600000000001 5.84 q2.24 0 3.46 0.91 t1.24 2.57 q0 1.22 -0.67 2.04 t-1.81 1.16 l0 0.04 q1.38 0.12 2.29 1.11 t0.91 2.37 q0 1.88 -1.42 2.91 t-4.04 1.05 l-4.28 0 l0 -14.16 l4.32 0 z M66.286 11.84 q1.54 0 2.34 -0.55 t0.8 -1.75 q0 -2.1 -2.96 -2.14 l-2.58 0 l0 4.44 l2.4 0 z M66.80600000000001 18.44 q1.52 0 2.43 -0.67 t0.91 -1.79 q0 -1.28 -0.9 -1.93 t-2.46 -0.65 l-2.9 0 l0 5.04 l2.92 0 z M87.7875 5.84 l0 1.56 l-7.22 0 l0 4.44 l6.74 0 l0 1.56 l-6.74 0 l0 5.04 l7.58 0 l0 1.56 l-9.26 0 l0 -14.16 l8.9 0 z M98.96900000000001 5.48 q2.58 0 3.96 1.66 l-1.38 1.2 q-0.8 -1.26 -2.58 -1.3 q-1.34 0 -2.18 0.64 t-0.84 1.74 q0 0.96 0.64 1.52 t2.4 1.1 q2.28 0.66 3.23 1.61 t0.95 2.57 q0 1.88 -1.4 3 t-3.58 1.14 q-1.46 0 -2.6 -0.52 t-1.76 -1.44 l1.44 -1.18 q0.46 0.76 1.28 1.17 t1.74 0.41 q1.26 0 2.17 -0.71 t0.91 -1.75 q0 -0.94 -0.53 -1.48 t-1.75 -0.92 l-1.62 -0.54 q-1.82 -0.62 -2.57 -1.55 t-0.75 -2.29 q0 -1.8 1.34 -2.93 t3.48 -1.15 z M119.5305 5.84 l0 1.56 l-4.68 0 l0 12.6 l-1.68 0 l0 -12.6 l-4.68 0 l0 -1.56 l11.04 0 z"></path>
                        </g>
                    </svg>
                </div>
                <p class="text-sm">Copyright © 2024 Motta, All rights reserved. <a class="font-semibold underline" href="">Privacy Notice Terms of Use</a></p>
            </div>
            <div class="flex gap-7">
                <div class="flex items-center gap-2">
                    <span><svg width="20" height="20" aria-hidden="true" role="img" focusable="false" viewBox="0 0 32 32">
                            <path d="M29.509 13.714c-0.137-0.777-0.32-1.554-0.594-2.286-1.737-4.891-6.149-8.503-11.474-9.074 0 0 0-0.023-0.023-0.023 0 0 0 0 0 0.023-0.457-0.046-0.937-0.069-1.417-0.069s-0.96 0.023-1.417 0.069c0 0 0 0 0-0.023 0 0 0 0.023-0.023 0.023-5.326 0.571-9.737 4.183-11.474 9.074-0.251 0.731-0.457 1.509-0.594 2.286-0.114 0.754-0.206 1.509-0.206 2.286s0.091 1.531 0.206 2.286c0.137 0.777 0.32 1.554 0.594 2.286 1.737 4.891 6.149 8.503 11.474 9.074 0 0 0 0.023 0.023 0.023 0 0 0 0 0-0.023 0.457 0.046 0.937 0.069 1.417 0.069s0.96-0.023 1.417-0.069c0 0 0 0 0 0.023 0 0 0-0.023 0.023-0.023 5.326-0.549 9.737-4.183 11.474-9.074 0.251-0.731 0.457-1.509 0.594-2.286 0.114-0.754 0.206-1.509 0.206-2.286s-0.091-1.531-0.206-2.286zM26.469 11.429h-5.211c-0.434-2.263-1.189-4.411-2.217-6.4 3.314 0.937 6.057 3.291 7.429 6.4zM19.429 16c0 0.777-0.046 1.531-0.114 2.286h-6.629c-0.069-0.754-0.114-1.509-0.114-2.286s0.046-1.531 0.114-2.286h6.606c0.091 0.754 0.137 1.509 0.137 2.286zM16.16 27.406c-0.069 0-0.114 0.023-0.16 0.023s-0.091-0.023-0.16-0.023c-1.28-2.080-2.24-4.366-2.766-6.834h5.874c-0.549 2.469-1.509 4.754-2.789 6.834zM15.84 4.594c0.069 0 0.114-0.023 0.16-0.023s0.091 0.023 0.16 0.023c1.28 2.080 2.24 4.366 2.766 6.834h-5.851c0.526-2.469 1.486-4.754 2.766-6.834zM12.96 5.029c-1.029 1.989-1.783 4.137-2.217 6.4h-5.211c1.371-3.109 4.114-5.463 7.429-6.4zM4.571 16c0-0.777 0.091-1.554 0.229-2.286h5.6c-0.069 0.754-0.114 1.509-0.114 2.286s0.046 1.531 0.114 2.286h-5.6c-0.137-0.731-0.229-1.509-0.229-2.286zM5.531 20.571h5.211c0.434 2.263 1.211 4.411 2.217 6.4-3.314-0.937-6.057-3.291-7.429-6.4zM19.040 26.971c1.029-1.989 1.783-4.137 2.217-6.4h5.189c-1.349 3.109-4.091 5.463-7.406 6.4zM21.6 18.286c0.069-0.754 0.114-1.509 0.114-2.286s-0.046-1.531-0.114-2.286h5.6c0.137 0.731 0.229 1.509 0.229 2.286s-0.091 1.554-0.229 2.286h-5.6z"></path>
                        </svg></span>
                    <p class="text-sm">English</p>
                </div>
                <div class="flex items-center gap-2">
                    <span class="motta-svg-icon motta-svg-icon--currency"><svg width="24" height="24" aria-hidden="true" role="img" focusable="false" viewBox="0 0 32 32">
                            <path d="M27.429 6.857h-22.857c-1.257 0-2.286 1.029-2.286 2.286v13.714c0 1.257 1.029 2.286 2.286 2.286h22.857c1.257 0 2.286-1.029 2.286-2.286v-13.714c0-1.257-1.029-2.286-2.286-2.286zM4.571 9.143h3.429c0 1.897-1.531 3.429-3.429 3.429v-3.429zM4.571 22.857v-3.429c1.897 0 3.429 1.531 3.429 3.429h-3.429zM27.429 22.857h-3.429c0-1.897 1.531-3.429 3.429-3.429v3.429zM27.429 17.143c-3.154 0-5.714 2.56-5.714 5.714h-11.429c0-3.154-2.56-5.714-5.714-5.714v-2.286c3.154 0 5.714-2.56 5.714-5.714h11.429c0 3.154 2.56 5.714 5.714 5.714v2.286zM27.429 12.571c-1.897 0-3.429-1.531-3.429-3.429h3.429v3.429z"></path>
                            <path d="M16 11.429c-2.514 0-4.571 2.057-4.571 4.571s2.057 4.571 4.571 4.571 4.571-2.057 4.571-4.571c0-2.514-2.057-4.571-4.571-4.571zM16 18.286c-1.257 0-2.286-1.029-2.286-2.286s1.029-2.286 2.286-2.286 2.286 1.029 2.286 2.286-1.029 2.286-2.286 2.286z"></path>
                        </svg></span>
                    <p>₹-INR</p>
                </div>
            </div>
        </div>
    </footer>
</body>

</html>